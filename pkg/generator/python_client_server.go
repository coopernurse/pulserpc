...
func (p *PythonClientServer) Generate(idl *parser.IDL, fs *flag.FlagSet) error {
	...
	serverCode := generateServerPy(idl, structMap, enumMap, interfaceMap, namespaceMap, baseDir, outputDir, string(idlJSON))
...
func generateServerPy(idl *parser.IDL, _ map[string]*parser.Struct, _ map[string]*parser.Enum, _ map[string]*parser.Interface, namespaceMap map[string]*NamespaceTypes, baseDir string, outputDir string, idlJson string) string {
	var sb strings.Builder
	...
	sb.WriteString("IDL_JSON = ")
	sb.WriteString(escapePythonTripleQuotedString(idlJson))
	sb.WriteString("\n")
	...
	sb.WriteString("            try:\n")
	sb.WriteString("                # Use embedded IDL JSON constant\n")
	sb.WriteString("                idl_doc = json.loads(IDL_JSON)\n")
	sb.WriteString("                \n")
