...
func (p *PythonClientServer) Generate(idl *parser.IDL, fs *flag.FlagSet) error {
	...
	if p.usePydantic {
		modelsCode := generateModelsPy(idl, namespaceMap, p.packageBase)
...
func (p *PythonClientServer) generateNamespaceFiles(namespaceMap map[string]*NamespaceTypes, paths PythonNamespacePaths, fs *flag.FlagSet) error {
	...
	for _, ns := range namespaces {
		...
		serverCode := generateServerPyForNamespace(nsTypes, p.packageBase)
		...
		clientCode := generateClientPyForNamespace(nsTypes, p.packageBase)
...
func (p *PythonClientServer) generateTestFiles(namespaceMap map[string]*NamespaceTypes, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, paths PythonNamespacePaths, fs *flag.FlagSet) error {
	...
	for _, ns := range namespaces {
		...
		testServerCode := generateTestServerPyForNamespace(nsTypes, structMap, enumMap, ns, p.packageBase)
		...
		testClientCode := generateTestClientPyForNamespace(nsTypes, structMap, enumMap, ns, p.packageBase)
...
// resolveRuntimeImport returns the Python import path for the pulserpc runtime.
// When packageBase is set, returns "packageBase.pulserpc".
// When packageBase is empty, returns "pulserpc" for backwards compatibility.
func resolveRuntimeImport(packageBase string) string {
	if packageBase != "" {
		return packageBase + ".pulserpc"
	}
	return "pulserpc"
}

// generateServerPyForNamespace generates server.py for a specific namespace
func generateServerPyForNamespace(nsTypes *NamespaceTypes, packageBase string) string {
	var sb strings.Builder

	runtimeImport := resolveRuntimeImport(packageBase)
...
func generateClientPyForNamespace(nsTypes *NamespaceTypes, packageBase string) string {
	var sb strings.Builder
	...
	sb.WriteString(fmt.Sprintf("from %s import HttpTransport, Client\n\n", runtimeImport))
...
// crossNamespaceRef tracks a type reference that crosses namespace boundaries
...
// collectCrossNamespaceRefs finds all user-defined type references that point to other namespaces
func collectCrossNamespaceRefs(nsTypes *NamespaceTypes, currentNS string) []crossNamespaceRef {
	refs := make([]crossNamespaceRef, 0)
	seen := make(map[string]bool)

	for _, structDef := range nsTypes.Structs {
		for _, fld := range structDef.Fields {
			collectRefsFromType(fld.Type, currentNS, &refs, seen)
		}
	}
...
// collectRefsFromType recursively extracts cross-namespace type references
func collectRefsFromType(t *parser.Type, currentNS string, refs *[]crossNamespaceRef, seen map[string]bool) {
	if t == nil {
		return
	}
	if t.IsArray() {
		collectRefsFromType(t.Array, currentNS, refs, seen)
	} else if t.IsMap() {
		collectRefsFromType(t.MapValue, currentNS, refs, seen)
	} else if t.IsUserDefined() {
		typeNS := GetNamespaceFromType(t.UserDefined, "")
		if typeNS != "" && typeNS != currentNS {
			key := typeNS + "." + GetBaseName(t.UserDefined)
			if !seen[key] {
				seen[key] = true
...
// buildCrossNamespaceImports generates Python import statements for cross-namespace references
func buildCrossNamespaceImports(refs []crossNamespaceRef, packageBase string) string {
	if len(refs) == 0 {
		return ""
	}

	// Group by target namespace
	nsTypes := make(map[string][]string)
	for _, ref := range refs {
		nsTypes[ref.TargetNS] = append(nsTypes[ref.TargetNS], ref.BaseName)
	}

	var sb strings.Builder
	for ns, types := range nsTypes {
		importPath := ns
		if packageBase != "" {
			importPath = packageBase + "." + ns
		}
		fmt.Fprintf(&sb, "from %s import %s\n", importPath, strings.Join(types, ", "))
	}

	return sb.String() + "\n"
}

// generateTypesPyForNamespace generates types.py for a specific namespace
func generateTypesPyForNamespace(nsTypes *NamespaceTypes, currentNS string, packageBase string) string {
	var sb strings.Builder

	sb.WriteString("# Generated by pulserpc - do not edit\n\n")
	sb.WriteString("# Data types and enums for this namespace\n\n")
	sb.WriteString("from dataclasses import dataclass, field\n")
	sb.WriteString("from typing import Optional, List, Dict, Any\n")

	// Collect and emit cross-namespace imports
	crossRefs := collectCrossNamespaceRefs(nsTypes, currentNS)
	crossImports := buildCrossNamespaceImports(crossRefs, packageBase)
	if crossImports != "" {
		sb.WriteString("\n")
		sb.WriteString("# Cross-namespace imports\n")
		sb.WriteString(crossImports)
	}

	sb.WriteString("\n")

	// Generate dataclasses for structs in this namespace
	for _, structDef := range nsTypes.Structs {
		writeTypesPyStruct(&sb, structDef)
	}
...
// writeTypesPyStruct generates a dataclass for a struct
func writeTypesPyStruct(sb *strings.Builder, structDef *parser.Struct) {
	...
	if structDef.Comment != "" {
		...
		fmt.Fprintf(sb, "    \"\"\"Data class for %s\"\"\"\n", baseName)
	}
	...
	for _, fld := range structDef.Fields {
		if fld.Comment != "" {
			lines := strings.Split(strings.TrimSpace(fld.Comment), "\n")
			for _, line := range lines {
				fmt.Fprintf(sb, "    # %s\n", line)
			}
		}
		fmt.Fprintf(sb, "    %s: ", fld.Name)
		writeTypesPyTypeAnnotation(sb, fld.Type, fld.Optional)
		if fld.Optional {
			sb.WriteString(" = None")
		} else {
...
// writeTypesPyEnum generates a class for an enum
func writeTypesPyEnum(sb *strings.Builder, enum *parser.Enum) {
	if enum.Comment != "" {
	...
	fmt.Fprintf(sb, "class %s:\n", baseName)
	if enum.Comment != "" {
		fmt.Fprintf(sb, "    \"\"\"%s\"\"\"\n", escapePythonDocstring(strings.TrimSpace(enum.Comment)))
	} else {
		fmt.Fprintf(sb, "    \"\"\"Enum for %s\"\"\"\n", baseName)
	}
...
// writeTypesPyTypeAnnotation writes a Python type annotation for types.py
func writeTypesPyTypeAnnotation(sb *strings.Builder, t *parser.Type, optional bool) {
	if t.IsBuiltIn() {
		switch t.BuiltIn {
		case "string":
			if optional {
		...
		sb.WriteString("List[")
		writeTypesPyTypeAnnotation(sb, t.Array, false)
		sb.WriteString("]")
	} else if t.IsMap() {
		sb.WriteString("Dict[str, ")
		writeTypesPyTypeAnnotation(sb, t.MapValue, false)
		sb.WriteString("]")
	} else if t.IsUserDefined() {
		typeName := t.UserDefined
...
func generateModelsPy(_ *parser.IDL, namespaceMap map[string]*NamespaceTypes, packageBase string) string {
	var sb strings.Builder
	...
	// Collect all cross-namespace refs across all namespaces
	allCrossRefs := make([]crossNamespaceRef, 0)
	seen := make(map[string]bool)
	for namespace, types := range namespaceMap {
		...
		for _, structDef := range types.Structs {
			for _, fld := range structDef.Fields {
				collectRefsFromTypeForPydantic(fld.Type, namespace, &allCrossRefs, seen)
			}
		}
	}

	// Emit cross-namespace imports at the top
	crossImports := buildCrossNamespaceImports(allCrossRefs, packageBase)
	if crossImports != "" {
		sb.WriteString("\n")
		sb.WriteString("# Cross-namespace imports\n")
		sb.WriteString(crossImports)
...
// collectRefsFromTypeForPydantic recursively extracts cross-namespace type references for Pydantic
func collectRefsFromTypeForPydantic(t *parser.Type, currentNS string, refs *[]crossNamespaceRef, seen map[string]bool) {
	if t == nil {
		return
	}
	if t.IsArray() {
		collectRefsFromTypeForPydantic(t.Array, currentNS, refs, seen)
	} else if t.IsMap() {
		collectRefsFromTypeForPydantic(t.MapValue, currentNS, refs, seen)
	} else if t.IsUserDefined() {
		typeNS := GetNamespaceFromType(t.UserDefined, "")
		if typeNS != "" && typeNS != currentNS {
			key := typeNS + "." + GetBaseName(t.UserDefined)
...
func generateTestServerPyForNamespace(nsTypes *NamespaceTypes, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, currentNS string, packageBase string) string {
	var sb strings.Builder
	runtimeImport := resolveRuntimeImport(packageBase)
...
func generateTestClientPyForNamespace(nsTypes *NamespaceTypes, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, currentNS string, packageBase string) string {
	var sb strings.Builder
	runtimeImport := resolveRuntimeImport(packageBase)
	...
	fmt.Fprintf(&sb, "from %s import HttpTransport, Client\n", runtimeImport)

	// Emit cross-namespace imports for types
	crossRefs := collectCrossNamespaceRefs(nsTypes, currentNS)
	crossImports := buildCrossNamespaceImports(crossRefs, packageBase)
	if crossImports != "" {
		sb.WriteString("\n")
