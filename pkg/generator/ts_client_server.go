...
func (p *TSClientServer) Generate(idl *parser.IDL, fs *flag.FlagSet) error {
	...
	// Generate server.ts with abstract interface classes only
...
func generateServerTs(idl *parser.IDL, _ map[string]*parser.Struct, _ map[string]*parser.Enum, _ map[string]*parser.Interface, packagePrefix string, _ map[string]*NamespaceTypes) string {
	var sb strings.Builder
