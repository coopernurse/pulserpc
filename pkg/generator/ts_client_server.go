...
func writeInterfaceStubTsForNamespace(sb *strings.Builder, iface *parser.Interface, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, currentNs string, allNamespaceMap map[string]*NamespaceTypes) {
	if iface.Comment != "" {
		lines := strings.Split(strings.TrimSpace(iface.Comment), "\n")
		for _, line := range lines {
			fmt.Fprintf(sb, "// %s\n", line)
...
func generateTestServerTs(idl *parser.IDL, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, _ map[string]*parser.Interface, outputDir string, namespaceMap map[string]*NamespaceTypes) string {
	var sb strings.Builder
