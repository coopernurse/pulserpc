...
func (p *GoClientServer) Generate(idl *parser.IDL, fs *flag.FlagSet) error {
	...
	for namespace, types := range namespaceMap {
		...
		namespaceCode := generateNamespaceGo(namespace, primaryNs, types, structMap, enumMap, namespaceMap, packageImportPath, runtimeImportPath)
...
func generateNamespaceGo(namespace string, primaryNs string, types *NamespaceTypes, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, namespaceMap map[string]*NamespaceTypes, packageImportPath string, runtimeImportPath string) string {
	...
	// For the primary namespace, also declare ALL_STRUCTS and ALL_ENUMS
	// fully populated with all types from all namespaces
	if namespace == primaryNs {
		sb.WriteString("\n// ALL_STRUCTS and ALL_ENUMS contain all types from all namespaces\n")
		sb.WriteString("var ALL_STRUCTS = pulserpc.StructMap{\n")
		for ns, nsTypes := range namespaceMap {
			for _, s := range nsTypes.Structs {
				structName := s.Name
				if ns != "" && !strings.Contains(structName, ".") {
				...
				for _, field := range s.Fields {
					...
					sb.WriteString(fmt.Sprintf("				\"name\": \"%s\",\n", field.Name))
					sb.WriteString("				\"type\": ")
					writeTypeDictGo(&sb, field.Type, ns)
					sb.WriteString(",\n")
					if field.Optional {
						sb.WriteString("				\"optional\": true,\n")
					}
		...
		sb.WriteString("var ALL_ENUMS = pulserpc.EnumMap{\n")
		for ns, nsTypes := range namespaceMap {
			for _, e := range nsTypes.Enums {
				enumName := e.Name
				if ns != "" && !strings.Contains(enumName, ".") {
...
func writeClientMethodGo(sb *strings.Builder, iface *parser.Interface, method *parser.Method, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, namespace string, nsImports map[string]string, packageImportPath string) {
	...
	if method.ReturnType != nil {
		...
		goReturnType2 := getGoTypeWithNamespace(method.ReturnType, structMap, enumMap, method.ReturnOptional, namespace, nsImports, packageImportPath)
