...
func (p *GoClientServer) Generate(idl *parser.IDL, fs *flag.FlagSet) error {
	...
	for namespace, types := range namespaceMap {
		...
		namespaceCode := generateNamespaceGo(namespace, primaryNs, types, structMap, enumMap, runtimeImportPath)
...
func generateNamespaceGo(namespace string, primaryNs string, types *NamespaceTypes, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, runtimeImportPath string) string {
