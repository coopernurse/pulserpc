...
func (p *CSharpClientServer) Generate(idl *parser.IDL, fs *flag.FlagSet) error {
	...
	// Initialize namespace paths helper
	paths := NewCSharpNamespacePaths(outputDir, p.packageBase)

	// Ensure namespace directories exist (Step 3: Generate Namespace Directory Scaffolding)
	if _, err := paths.EnsureAllNamespaceDirs(namespaceMap); err != nil {
		return fmt.Errorf("failed to create namespace directories: %w", err)
	}

	// Copy runtime library files to the correct location
	// When packageBase is set, runtime goes to package-based path (e.g., MyApp/Lib/Rpc/PulseRPC)
	runtimeRelPath, err := filepath.Rel(outputDir, paths.ResolveRuntimeDir())
	if err != nil {
		runtimeRelPath = "PulseRPC"
	}
	if err := runtime.CopyRuntimeFilesToPackage("csharp", outputDir, runtimeRelPath, isSilent()); err != nil {
		return fmt.Errorf("failed to copy runtime files: %w", err)
	}

	// Get sorted list of all namespaces
	namespaces := CollectSortedNamespaces(namespaceMap)

	// Generate Contract.cs (shared interfaces and IdlData) in the output directory
	contractCode := generateContractCs(idl, structMap, enumMap, namespaceMap, paths)
	...
	// Generate one file per namespace in their respective directories
	for _, namespace := range namespaces {
		if namespace == "" {
			continue
		}
		types := namespaceMap[namespace]
		if types == nil {
			continue
		}
		namespaceCode := generateNamespaceCs(namespace, namespaces, types, structMap, enumMap, namespaceMap, paths)
		namespacePath := paths.ResolveOutputPath(namespace, NamespaceToPascalCase(namespace)+".cs")
		...
		// Generate Types.cs in namespace directory
		typesCode := generateTypesCs(namespace, types, structMap, enumMap, namespaceMap, paths)
		typesPath := paths.ResolveOutputPath(namespace, "Types.cs")
		if err := os.WriteFile(typesPath, []byte(typesCode), 0644); err != nil {
			return fmt.Errorf("failed to write Types.cs for namespace %s: %w", namespace, err)
		}
		PrintFileCreated(typesPath, fs)

		// Generate Server.cs in namespace directory
		serverNsCode := generateNamespaceServerCs(namespace, types, structMap, enumMap, namespaceMap, paths)
		serverNsPath := paths.ResolveOutputPath(namespace, "Server.cs")
		if err := os.WriteFile(serverNsPath, []byte(serverNsCode), 0644); err != nil {
			return fmt.Errorf("failed to write Server.cs for namespace %s: %w", namespace, err)
		}
		PrintFileCreated(serverNsPath, fs)

		// Generate Client.cs in namespace directory
		clientNsCode := generateNamespaceClientCs(namespace, types, structMap, enumMap, namespaceMap, paths)
		clientNsPath := paths.ResolveOutputPath(namespace, "Client.cs")
		if err := os.WriteFile(clientNsPath, []byte(clientNsCode), 0644); err != nil {
			return fmt.Errorf("failed to write Client.cs for namespace %s: %w", namespace, err)
		}
		PrintFileCreated(clientNsPath, fs)
	}
	...
	// Generate Server.cs
	serverCode := generateServerCs(idl, namespaceMap, structMap, enumMap, string(jsonData), paths)
	...
	// Generate Client.cs
	clientCode := generateClientCs(idl, structMap, enumMap, namespaceMap, string(jsonData), paths)
	...
	if generateTestServer {
		...
		testServerCode := generateTestServerCs(idl, namespaces, structMap, enumMap, paths)
		...
		testClientCode := generateTestClientCs(idl, namespaces, structMap, enumMap, paths)
...
func generateNamespaceCs(namespace string, allNamespaces []string, types *NamespaceTypes, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, namespaceMap map[string]*NamespaceTypes, paths CSharpNamespacePaths) string {
	...
	generateStructClassesCs(&sb, types.Structs, structMap, enumMap, namespace, namespaceMap, "    ")
	sb.WriteString("\n")
...
func mapTypeToCsType(t *parser.Type, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, optional bool) string {
	if t.IsBuiltIn() {
		...
		typeName := getStructOrEnumTypeNameSimple(t.UserDefined, structMap, enumMap)
		if optional {
			return typeName + "?"
		}
		return typeName
...
// mapTypeToCsTypeWithNamespace maps an IDL type to a C# type string with namespace awareness
func mapTypeToCsTypeWithNamespace(t *parser.Type, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, optional bool, currentNs string, allNamespaceMap map[string]*NamespaceTypes) string {
	if t.IsBuiltIn() {
		csType := ""
		switch t.BuiltIn {
		case "string":
		...
		elementType := mapTypeToCsTypeWithNamespace(t.Array, structMap, enumMap, false, currentNs, allNamespaceMap)
		return "List<" + elementType + ">"
	} else if t.IsMap() {
		valueType := mapTypeToCsTypeWithNamespace(t.MapValue, structMap, enumMap, false, currentNs, allNamespaceMap)
		return "Dictionary<string, " + valueType + ">"
	} else if t.IsUserDefined() {
		typeName := getStructOrEnumTypeName(t.UserDefined, structMap, enumMap, currentNs, allNamespaceMap)
		if optional {
			return typeName + "?"
		}
		return typeName
...
// When in multi-namespace mode (namespaceMap provided), it adds namespace prefix for cross-namespace types
func getStructOrEnumTypeName(typeName string, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, currentNs string, _ map[string]*NamespaceTypes) string {
	baseName := GetBaseName(typeName)

	// Check if it's a struct in the current namespace or another namespace
	if s, ok := structMap[baseName]; ok {
		if currentNs != "" && s.Namespace != "" && s.Namespace != currentNs {
			// Cross-namespace reference - use namespace prefix (PascalCase)
			nsPascal := NamespaceToPascalCase(s.Namespace)
			return nsPascal + "." + baseName
		}
		return baseName
	}
	...
	// Check if it's an enum in the current namespace or another namespace
	if e, ok := enumMap[baseName]; ok {
		if currentNs != "" && e.Namespace != "" && e.Namespace != currentNs {
			nsPascal := NamespaceToPascalCase(e.Namespace)
			return nsPascal + "." + baseName
		}
		return baseName
	}
	if e, ok := enumMap[typeName]; ok {
		if currentNs != "" && e.Namespace != "" && e.Namespace != currentNs {
			nsPascal := NamespaceToPascalCase(e.Namespace)
			return nsPascal + "." + baseName
		}
...
// getStructOrEnumTypeNameSimple returns the C# type name without namespace awareness
func getStructOrEnumTypeNameSimple(typeName string, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum) string {
	return getStructOrEnumTypeName(typeName, structMap, enumMap, "", nil)
}
...
func generateStructClassesCs(sb *strings.Builder, structs []*parser.Struct, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, currentNs string, allNamespaceMap map[string]*NamespaceTypes, prefix string) {
	for _, s := range structs {
		...
		for _, field := range s.Fields {
			...
			// Property type - use namespace-aware mapping
			csType := mapTypeToCsTypeWithNamespace(field.Type, structMap, enumMap, field.Optional, currentNs, allNamespaceMap)
...
func generateContractCs(idl *parser.IDL, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, namespaceMap map[string]*NamespaceTypes, paths CSharpNamespacePaths) string {
	var sb strings.Builder
...
func generateServerCs(idl *parser.IDL, namespaceMap map[string]*NamespaceTypes, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, idlJson string, paths CSharpNamespacePaths) string {
	var sb strings.Builder
	...
	for _, ns := range namespaces {
		...
		sb.WriteString(fmt.Sprintf("using static %s%s.%sIdl;\n", prefix, ns, ns))
		sb.WriteString(fmt.Sprintf("using %s%s;\n", prefix, ns))
	}
	sb.WriteString("\n")
...
func generateClientCs(idl *parser.IDL, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, namespaceMap map[string]*NamespaceTypes, idlJson string, paths CSharpNamespacePaths) string {
	var sb strings.Builder
	...
	sb.WriteString("namespace PulseRPC\n")
	sb.WriteString("{\n")
	sb.WriteString("    internal static class IdlHolder\n")
	sb.WriteString("    {\n")
	...
	sb.WriteString("        private static readonly string IDL_JSON = ")
	sb.WriteString(escapeCSharpVerbatimString(idlJson))
	sb.WriteString(";\n\n")
	sb.WriteString("        public static string GetIDLJson() => IDL_JSON;\n\n")
	sb.WriteString("        public static string ExtractChecksum(object? idl)\n")
	sb.WriteString("        {\n")
	sb.WriteString("            if (idl is JsonElement element && element.TryGetProperty(\"checksum\", out var checksumProp))\n")
	sb.WriteString("            {\n")
	sb.WriteString("                return checksumProp.GetString() ?? string.Empty;\n")
	sb.WriteString("            }\n")
	sb.WriteString("            return string.Empty;\n")
	sb.WriteString("        }\n")
	sb.WriteString("    }\n\n")
...
func writeInterfaceClientCs(sb *strings.Builder, iface *parser.Interface, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum) {
	...
	sb.WriteString("    private IContractAuditor _auditor;\n")
	sb.WriteString("    private bool _verifyOnBootstrap;\n")
	sb.WriteString("    private object? _localIDL;\n\n")
	...
	sb.WriteString("    public " + clientClassName + " WithAuditor(IContractAuditor auditor)\n")
	sb.WriteString("    {\n")
	sb.WriteString("        _auditor = auditor;\n")
	sb.WriteString("        return this;\n")
	sb.WriteString("    }\n\n")

	sb.WriteString("    public " + clientClassName + " VerifyOnBootstrap()\n")
	sb.WriteString("    {\n")
	sb.WriteString("        _verifyOnBootstrap = true;\n")
	sb.WriteString("        return this;\n")
	sb.WriteString("    }\n\n")

	sb.WriteString("    public void SetLocalIDL(string idlJson)\n")
	sb.WriteString("    {\n")
	sb.WriteString("        _localIDL = JsonSerializer.Deserialize<object>(idlJson);\n")
	sb.WriteString("    }\n\n")

	sb.WriteString("    public async Task<VerificationResult> VerifyCompatibility(CancellationToken cancellationToken = default)\n")
	sb.WriteString("    {\n")
	sb.WriteString("        object? clientIDL;\n")
	sb.WriteString("        if (_localIDL != null)\n")
	sb.WriteString("        {\n")
	sb.WriteString("            clientIDL = _localIDL;\n")
	sb.WriteString("        }\n")
	sb.WriteString("        else\n")
	sb.WriteString("        {\n")
	sb.WriteString("            clientIDL = JsonSerializer.Deserialize<object>(IdlHolder.GetIDLJson());\n")
	sb.WriteString("        }\n\n")
	sb.WriteString("        var idlObj = JsonSerializer.Deserialize<object>(IdlHolder.GetIDLJson());\n")
	sb.WriteString("        var deltas = DiffEngine.DiffIDL(clientIDL, idlObj);\n\n")
	sb.WriteString("        var compatible = true;\n")
	sb.WriteString("        foreach (var delta in deltas)\n")
	sb.WriteString("        {\n")
	sb.WriteString("            if (delta.Severity == Severity.Error)\n")
	sb.WriteString("            {\n")
	sb.WriteString("                compatible = false;\n")
	sb.WriteString("                break;\n")
	sb.WriteString("            }\n")
	sb.WriteString("        }\n\n")
	sb.WriteString("        var result = new VerificationResult\n")
	sb.WriteString("        {\n")
	sb.WriteString("            Compatible = compatible,\n")
	sb.WriteString("            ServerChecksum = IdlHolder.ExtractChecksum(idlObj),\n")
	sb.WriteString("            ClientChecksum = IdlHolder.ExtractChecksum(clientIDL),\n")
	sb.WriteString("            Deltas = deltas,\n")
	sb.WriteString("            Timestamp = DateTime.UtcNow\n")
	sb.WriteString("        };\n\n")
	sb.WriteString("        if (_auditor != null)\n")
	sb.WriteString("        {\n")
	sb.WriteString("            _auditor.Audit(result);\n")
	sb.WriteString("        }\n\n")
...
func generateTestServerCs(idl *parser.IDL, allNamespaces []string, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, paths CSharpNamespacePaths) string {
	var sb strings.Builder
...
func generateTestClientCs(idl *parser.IDL, allNamespaces []string, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, paths CSharpNamespacePaths) string {
	var sb strings.Builder
...
// generateTypesCs generates Types.cs for a namespace (contains enums and structs)
func generateTypesCs(namespace string, types *NamespaceTypes, structMap map[string]*parser.Struct, enumMap map[string]*parser.Enum, namespaceMap map[string]*NamespaceTypes, paths CSharpNamespacePaths) string {
	var sb strings.Builder
	...
	// Add imports for other namespaces when package base is set
	prefix := paths.GetNamespaceImportPrefix()
	for _, ns := range CollectSortedNamespaces(namespaceMap) {
		if ns != "" && ns != namespace {
			nsPascal := NamespaceToPascalCase(ns)
			sb.WriteString(fmt.Sprintf("using %s%s;\n", prefix, nsPascal))
		}
	}
	sb.WriteString("\n")
...
// generateNamespaceServerCs generates Server.cs for a namespace
func generateNamespaceServerCs(namespace string, _ *NamespaceTypes, _ map[string]*parser.Struct, _ map[string]*parser.Enum, _ map[string]*NamespaceTypes, paths CSharpNamespacePaths) string {
	var sb strings.Builder
	...
	sb.WriteString("    public class NamespaceServer\n")
	sb.WriteString("    {\n")
	sb.WriteString("        public static string Namespace => \"" + namespace + "\";\n")
	sb.WriteString("    }\n")
...
// generateNamespaceClientCs generates Client.cs for a namespace
func generateNamespaceClientCs(namespace string, _ *NamespaceTypes, _ map[string]*parser.Struct, _ map[string]*parser.Enum, _ map[string]*NamespaceTypes, paths CSharpNamespacePaths) string {
	var sb strings.Builder
	...
	sb.WriteString("    public class NamespaceClient\n")
	sb.WriteString("    {\n")
	sb.WriteString("        public static string Namespace => \"" + namespace + "\";\n")
	sb.WriteString("    }\n")
