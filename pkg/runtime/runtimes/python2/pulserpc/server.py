...
class Server(object):
    ...
    def __init__(
        self, contract, validate_requests=True, validate_responses=True, on_error=None
    ):
        """Initialize Server"""
        self.handlers = {}
    ...
    def call(self, req, ctx=None):
        ...
        if not method or not isinstance(method, _string_types):
            return self._error_response(
                req.get("id"), -32600, "Invalid Request", "Method must be a string"
            )

        if method == "pulserpc-idl":
            req_id = req.get("id")
            return {"jsonrpc": "2.0", "result": self.contract.idl_parsed, "id": req_id}
        ...
        if iface_name not in self.handlers:
            return self._error_response(
                req_id, -32601, "Method not found", "Unknown interface: %s" % iface_name
            )
        ...
        if not hasattr(handler, func_name):
            return self._error_response(
                req_id, -32601, "Method not found", "Unknown method: %s" % method
            )
        ...
        if not isinstance(params, list):
            return self._error_response(
                req_id, -32602, "Invalid params", "Parameters must be an array"
            )
        ...
        try:
            ...
            return self._error_response(
                req_id, -32602, "Invalid params", "Parameter mismatch: %s" % e
            )
        except Exception as e:
            if isinstance(e, RPCError):
                return self._error_response(
                    req_id, e.code, getattr(e, "message"), e.data
                )
            else:
                if self.on_error is not None:
                    ...
                    traceback.print_exc()
                return self._error_response(
                    req_id, -32603, "Internal error", "Handler exception: %s" % e
                )
