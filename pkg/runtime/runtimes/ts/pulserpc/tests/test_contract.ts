...
import { Contract, InterfaceImpl, NoOpAuditor, FailFastAuditor } from "../contract.js";
...
function testNoOpAuditor() {
  ...
  assert.doesNotThrow(() => a.audit({ compatible: true, deltas: [], serverChecksum: "", clientChecksum: "", timestamp: new Date() }));
...
function testFailFastAuditorThrowsOnIncompatible() {
  ...
  assert.throws(() => a.audit({ compatible: false, deltas: [{ severity: "Error" as any, entityType: "test" as any, entityName: "", memberName: "", changeType: "Added" as any, direction: "Mismatch" as any, description: "boom" }], serverChecksum: "", clientChecksum: "", timestamp: new Date() }), /Contract compatibility/);
  assert.doesNotThrow(() => a.audit({ compatible: true, deltas: [], serverChecksum: "", clientChecksum: "", timestamp: new Date() }));
